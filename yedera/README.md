...
```bash
./yedera/build/yedera --model-path /path/to/model.gguf --prompt "Say hello in five words."
```

Interactive chat using the model configured in `yedera.conf`:

```bash
./yedera/build/yedera --interactive
```
...
~/.local/bin/yedera --prompt "Summarize this project in one sentence."
```

Direct GGUF path override:

```bash
./yedera/build/yedera --model-path /path/to/model.gguf --prompt "Explain GGUF in one sentence."
```
...
For local RAG, point `model_embeddings` at a local GGUF embeddings model and `rag_documents_path` at a file or directory of text documents. Relative paths in `yedera.conf` resolve from the config file directory, so `model_embeddings = "model/...gguf"` and `rag_documents_path = "rag"` work next to the binary.
