...
elseif (CASE STREQUAL "no_prompt_chat")
    file(WRITE "${TEST_DIR}/yedera.conf"
        ...
        "debug = true\n"
        "verbose = false\n")
    file(WRITE "${TEST_DIR}/stdin.txt" "Reply with one word.\n")
    ...
    expect_contains("${output}" "> " "missing prompt should enter chat mode and print the chat prompt")
...
elseif (CASE STREQUAL "rag_startup")
    file(MAKE_DIRECTORY "${TEST_DIR}/rag/nested")
    file(WRITE "${TEST_DIR}/rag/note.md" "garage retrieval keyword\n\nThe answer lives in the local note file.\n")
    file(WRITE "${TEST_DIR}/rag/odd.txt" "the obsidian teacup is catalogued under shelf delta\n")
    file(WRITE "${TEST_DIR}/rag/nested/deep.md" "nested startup keyword\n\nThis nested file should be indexed recursively at startup.\n")
    file(WRITE "${TEST_DIR}/yedera.conf"
        "prompt = \"You are a test assistant.\"\n"
        "model_path = \"${MODEL_PATH}\"\n"
    ...
    expect_contains("${output}" "[debug] RAG mode enabled" "RAG startup test should enable RAG")
    expect_contains("${output}" "[rag] ${TEST_DIR}/rag/note.md" "RAG startup should learn top-level markdown files")
    expect_contains("${output}" "[rag] ${TEST_DIR}/rag/odd.txt" "RAG startup should learn top-level text files")
    expect_contains("${output}" "[rag] ${TEST_DIR}/rag/nested/deep.md" "RAG startup should recurse into nested directories")
    expect_contains("${output}" "[retrieved " "RAG startup should retrieve indexed chunks")
elseif (CASE STREQUAL "startup_rag_disabled")
    ...
    expect_contains("${output}" "[debug] RAG: startup indexing disabled" "startup RAG should be disabled unless index_at_startup is configured")
    expect_not_contains("${output}" "[rag] ${TEST_DIR}/rag/note.md" "startup RAG should not learn files when index_at_startup is omitted")
    expect_not_contains("${output}" "[retrieved " "startup RAG should not retrieve chunks when startup indexing is disabled")
elseif (CASE STREQUAL "index_at_startup")
    file(MAKE_DIRECTORY "${TEST_DIR}/rag/nested")
    file(WRITE "${TEST_DIR}/rag/alpha.txt" "alpha startup token: saffron gate.\n")
    file(WRITE "${TEST_DIR}/rag/beta.md" "beta startup token: basil arch.\n")
    file(WRITE "${TEST_DIR}/rag/nested/gamma.txt" "gamma startup token: copper stair.\n")
    file(WRITE "${TEST_DIR}/yedera.conf"
        "prompt = \"You are a test assistant.\"\n"
        "model_path = \"${MODEL_PATH}\"\n"
        "model_embeddings = \"${MODEL_PATH}\"\n"
        "rag_documents_path = \"${TEST_DIR}/rag\"\n"
        "index_at_startup = *.txt\n"
        "n_gpu_layers = 0\n"
        "debug = true\n"
        "verbose = false\n")
    ...
    run_yedera(
        ...
        STATUS status
        WORKING_DIRECTORY "${TEST_DIR}"
        ARGS --config "${TEST_DIR}/yedera.conf" "alpha startup token" --n-predict 8 --temperature 0 --top-p 1 --min-p 0 --seed 1)

    expect_contains("${output}" "[rag] ${TEST_DIR}/rag/alpha.txt" "index_at_startup should include matching top-level files")
    expect_contains("${output}" "[rag] ${TEST_DIR}/rag/nested/gamma.txt" "index_at_startup should match nested files recursively")
    expect_not_contains("${output}" "[rag] ${TEST_DIR}/rag/beta.md" "index_at_startup should exclude files outside the configured patterns")
    expect_contains("${output}" "[retrieved " "index_at_startup should still retrieve indexed chunks")
elseif (CASE STREQUAL "learn_multi_entry_syntax")
    ...
    expect_contains("${output}" "[retrieved " "learn-mode multi-entry syntax should still leave chat retrieval working")
else()
